...
import { copySmall, listView, tool, trash } from '@wordpress/icons';
import {
 ...
 * Use the filtered keyboard shortcut from PHP or fall back to default
 */
const keyboardShortcut = window.classactConfig?.keyboardShortcut || {
	modifier: 'alt',
	character: 'c',
};
					...
					<InspectorAdvancedControls>
						{ classManagement.errorMessage && (
							<Notice
								...
								isDismissible
								onRemove={ () =>
									classManagement.setErrorMessage( '' )
								}
								className="classact-error-message"
							>
								<span id="classact-inspector-error">
									{ classManagement.errorMessage }
								</span>
							</Notice>
						) }
						...
						<div className="classact-button-group">
							<CopyButton
								className={ blockAttrs?.attributes?.className }
							/>
							<SortButton
								classesArray={ classManagement.classesArray }
								handleClassChange={
									classManagement.handleClassChange
								}
							/>
							<ClearButton
...
addFilter( 'editor.BlockEdit', FILTER_NAME, withClassActInspectorControls );
